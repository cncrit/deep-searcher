...
from deepsearcher.utils.web_visit import search_360, search_sogou, search_baidu
import random
...
def perform_retrieve(query: str=  Query(
    ...
    try:
        # 第一步获取网络搜索结果，storm格式。
        this_search = random.choice([search_360, search_sogou, search_baidu])
        web_results = this_search(query)
        print('已完成网络搜索，开始向量知识库搜索')
        # 第二步获取向量知识库结果，转为storm格式。
